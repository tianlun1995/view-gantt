...
// CONCATENATED MODULE: ./src/pages/wl-gantt/index.vue?vue&type=template&id=38a0ac20&
...
/* harmony default export */ var wl_ganttvue_type_script_lang_js_ = ({
  ...
  methods: {
    //隐藏按钮插槽
    showSoltBtn: function showSoltBtn(index) {
    ...
    parentStartDateToChild: function parentStartDateToChild(item) {// if (!item._parent) return;
    ...
    childEndDateToParent: function childEndDateToParent(item) {// if (!Array.isArray(item[this.selfProps.children])) return;
    ...
    handleMouseEnter: function handleMouseEnter(row, column, cell, event) {
      this.showIndex = row.indexs; // debugger
